<?php
/**
 * API para capturar dados reais da tabela dispositivos_faciais
 * Busca leituras do tipo 'culto' e processa automaticamente
 */
...
try {
    // Verificar se a tabela dispositivos_faciais existe
    $stmt_check = $conn->prepare("SHOW TABLES LIKE 'dispositivos_faciais'");
    $stmt_check->execute();
    $resultado_check = $stmt_check->get_result();
    $tabela_existe = $resultado_check->num_rows > 0;
    $stmt_check->close();
    ...
    if ($tabela_existe) {
        // Buscar leituras não processadas da tabela dispositivos_faciais
        $data_hoje = date('Y-m-d');
        $stmt = $conn->prepare("SELECT * FROM dispositivos_faciais WHERE tipo_dispositivo = 'culto' AND data_leitura >= ? AND processado = 0 ORDER BY data_leitura DESC LIMIT 10");
        ...
        if ($stmt) {
            ...
            while ($row = $resultado->fetch_assoc()) {
                ...
                // Converter para formato de evento
                $evento = [
                    'user_id' => $row['id_usuario'],
                    'card' => $row['cartao'] ?? null,
                    'face' => $row['face_id'] ?? null,
                    'event_type' => $row['tipo_leitura'] ?? 'FaceRecognition',
                    'result' => 'Pass',
                    'time' => $row['data_leitura'],
                    'raw' => $row
                ];
                ...
                if (processarLeituraFacial($evento)) {
                    ...
                    $stmt_update = $conn->prepare("UPDATE dispositivos_faciais SET processado = 1 WHERE id = ?");
                    if ($stmt_update) {
            ...
            logLeituraFacial("ERRO: Falha ao preparar query para dispositivos_faciais: " . $conn->error);
        }
    } else {
        logLeituraFacial("AVISO: Tabela dispositivos_faciais não encontrada. Monitor funcionando sem dados do dispositivo.");
    }
    ...
    // Log da captura
    logLeituraFacial("Monitor consultado - Total de leituras: $total_leituras, Novas leituras: " . count($leituras));
    ...
    echo json_encode([
        ...
        'logs' => $logs,
        'eventos' => $eventos,
        'leituras_processadas' => count($leituras),
        'used' => [
            'method' => 'GET',
            'timezone_format' => 'local',
            'name' => 'dispositivos_faciais'
        ]

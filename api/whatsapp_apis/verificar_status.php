<?php
/**
 * API - Verificar Status da Sessão WhatsApp
 */
...
try {
    ...
    if (!$url_parts || !isset($url_parts['scheme']) || !isset($url_parts['host'])) {
        throw new Exception('URL de mensagem inválida');
    }
    ...
    if (empty($nome_sessao)) {
        throw new Exception('Nome da sessão não encontrado');
    }
    ...
    if ($http_code === 200) {
        ...
        // Log para debug
        error_log("Resposta status-session: " . $response_data);
        ...
        if ($json_data) {
            ...
            if ($status_raw !== null) {
                ...
                // Se não encontrou campo de status
                if (empty($json_data)) {
                    $status_sessao = 'erro';
                ...
                $detalhes = ['response_raw' => $response_data, 'json_data' => $json_data];
            }
        } else {
            ...
            if (!empty($response_data)) {
                ...
                $detalhes = ['response_raw' => $response_data];
            } else {
                $status_sessao = 'erro';
                $mensagem = 'Resposta vazia do servidor';
                $detalhes = ['response_raw' => $response_data];
        ...
        $status_sessao = 'erro';
        $error_msg = isset($resultado['error']) ? $resultado['error'] : 'Erro ao conectar com o servidor';
        $mensagem = 'Erro de conexão: ' . $error_msg;
    ...
    echo json_encode([
        ...
        'mensagem' => $mensagem,
        'http_code' => $http_code,
        'url_verificada' => $status_url,
        'detalhes' => $detalhes
    ]);
    
} catch (Exception $e) {
    error_log("Erro em whatsapp_apis/verificar_status.php: " . $e->getMessage());
    echo json_encode([
        'status' => 'erro',

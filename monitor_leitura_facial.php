...
<!--
    Navegação removida - Esta página não requer autenticação
-->
    ...
    <script>
        ...
        function atualizarLogs(logs) {
            ...
            // Filtrar logs relevantes de leitura facial
            // Prioridade: _DoorFace_ e "Leitura recebida" são os mais importantes
            const logsFiltrados = logs.filter(function(log) {
                const mensagem = log.mensagem || '';
                
                // PRIORIDADE 1: Log do evento _DoorFace_ (informação crucial)
                if (mensagem.includes('_DoorFace_')) {
                    return true;
                }
                
                // PRIORIDADE 2: Log de "Leitura recebida" (com seta → para destacar)
                if (mensagem.includes('→ Leitura recebida')) {
                    return true;
                }
                
                // IGNORAR: Erros cURL repetitivos (timeout é normal)
                if (mensagem.includes('Erro cURL') && mensagem.includes('Operation timed out')) {
                    return false;
                }
                
                // PRIORIDADE 3: Logs de erros críticos (não cURL)
                if (mensagem.includes('ERRO') && !mensagem.includes('cURL')) {
                    return true;
                }
